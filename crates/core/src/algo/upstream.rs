...
pub struct UpstreamUnits {
    ...
    downstream: HashMap<UnitId, UnitId>,
}
...
impl UpstreamUnits {
    ...
    /// Return the unit immediately downstream of `id` inside this traversal.
    ///
    /// This is the traversal-tree parent: the unit through which `id` was
    /// first reached from the terminal. The terminal returns `None`, and so
    /// does any ID outside this set.
    ///
    /// Following this edge from every member reconstructs the drainage tree of
    /// the delineated watershed, which is what Strahler ordering and
    /// bifurcation ratios are computed from. On a DAG fabric a unit can have
    /// more than one downstream neighbour; this reports the one the traversal
    /// actually used. That choice is deterministic for a given graph and
    /// terminal, but it is not a claim of hydrologic uniqueness.
    pub fn downstream_of(&self, id: UnitId) -> Option<UnitId> {
...
pub fn collect_upstream(
    ...
    let mut downstream: HashMap<UnitId, UnitId> = HashMap::new();
    ...
    while let Some(current) = queue.pop_front() {
        ...
        if let Some(row) = graph.get(current) {
            for &upstream_id in row.upstream_ids() {
                ...
                downstream.insert(upstream_id, current);
...
mod tests {
    ...
    fn partial_eq_is_set_based() {
        ...
        // Equality is membership-based, so the traversal-parent map is not
        // part of it; these hand-built fixtures carry none.
        let a = UpstreamUnits {
            units: units_a,
            index: index.clone(),
    ...
    #[test]
    fn downstream_of_reports_the_traversal_parent() {
        // 1 → 2 → 3, delineating at terminal 3.
        let g = graph(&[(1, &[]), (2, &[1]), (3, &[2])]);
        let up = collect_upstream(aid(3), &g).unwrap();

        assert_eq!(up.downstream_of(aid(3)), None, "terminal has no parent");
        assert_eq!(up.downstream_of(aid(2)), Some(aid(3)));
        assert_eq!(up.downstream_of(aid(1)), Some(aid(2)));
        assert_eq!(up.downstream_of(aid(99)), None, "non-member has no parent");
    }

    #[test]
    fn downstream_edges_cover_every_unit_but_the_terminal() {
        // Confluence: 3 and 4 both drain into 2, which drains into 1.
        let g = graph(&[(1, &[2]), (2, &[3, 4]), (3, &[]), (4, &[])]);
        let up = collect_upstream(aid(1), &g).unwrap();

        let parented = up
            .iter()
            .filter(|id| up.downstream_of(**id).is_some())
            .count();
        assert_eq!(parented, up.len() - 1);
        assert_eq!(up.downstream_of(aid(3)), Some(aid(2)));
        assert_eq!(up.downstream_of(aid(4)), Some(aid(2)));
    }
}
