...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // A connection gets its own thread so a socket that never sends a request
    // cannot wedge the accept loop. Browsers routinely open such speculative
    // sockets, and a single-threaded loop blocks on the first one forever.
    // Delineation itself stays serialized behind the engine mutex — it is the
    // bottleneck, and one at a time is the honest capacity of this binary.
...
fn handle(
    ...
    match path {
        ...
        "/api/meta" => {
            let body = json!({ "dataset": dataset, "open_seconds": open_secs });
